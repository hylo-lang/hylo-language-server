...
final class LanguageServerTests: XCTestCase {
  ...
  func testSemanticTokens() throws {
    ...
    let sampleTypes: [HyloSemanticTokenType] = [.function, .keyword, .string, .number, .identifier]
    XCTAssertEqual(sampleTypes.count, 5)
    XCTAssertEqual(HyloSemanticTokenType.function.description, "function")
    XCTAssertEqual(HyloSemanticTokenType.keyword.description, "keyword")
