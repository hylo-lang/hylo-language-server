...
public func extractHoverContent(_ contents: ThreeTypeOption<MarkedString, [MarkedString], MarkupContent>) -> String {
  switch contents {
...
public func assertDefinitionAt(
  ...
  guard let unwrappedDefinition = definition else {
    throw TestError.unexpectedNil(
      message: "Expected definition at range \(expectedRange)\(expectedUri.map { " in \($0)" } ?? "") but got nil",
      file: file,
      line: line
    )
  }
