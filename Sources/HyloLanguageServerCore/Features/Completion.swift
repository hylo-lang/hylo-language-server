...
extension SourceFile {
  ...
  /// Returns the index corresponding to `p`, which is a position in `self`
  public func index(p: Position) -> SourceFile.Index {
    self.index(line: p.line + 1, column: p.character + 1)
  }
}

extension SourcePosition {
  /// Create a `SourcePosition` from `p`, a LSP `Position` in `s`
  public init(_ p: Position, in s: SourceFile) {
    self.init(s.index(p: p), in: s)
  }
}
...
extension HyloRequestHandler {
  ...
  public func completion(
    ...
    await withAnalyzedDocument(
      params.textDocument, fn: { await completionResponse(from: $0, with: params) })
  }

  /// Returns a completion response created from the `AnalyzedDocument` using `CompletionParams`
  private func completionResponse(
    from d: AnalyzedDocument, with ps: CompletionParams
  ) async -> Response<CompletionResponse> {
    do {
      logger.debug("Here we go again !")
      return try await handleCompletion(from: d, with: ps)
    } catch let e as JSONFailure {
      logger.debug("Ah shit, here we go again")
      return jsonFailure(message: e.message, code: e.code)
    } catch let e {
      return jsonFailure(message: e.localizedDescription)
    }
  }

  /// Returns a completion response from a document and a position therein
  private func handleCompletion(from document: AnalyzedDocument, with params: CompletionParams)
    async throws -> Response<CompletionResponse>
  {

    let u =
      try AbsoluteUrl(fromUrlString: params.textDocument.uri)
      ?? JSONFailure(
        "Invalid document uri: \(params.textDocument.uri)", code: ErrorCodes.InvalidParams)
    let input =
      try document.program.findSourceContainer(u, logger: logger)
      ?? JSONFailure("Failed to locate translation unti : \(params.textDocument.uri)")
    ...
    let (program, source) =
      await insertDummyNodeIfNeeded(
        in: input.source, at: input.source.index(p: params.position), program: document.program,
        url: u) ?? (document.program, input.source)

    guard
      let node = program.innermostTree(
        containing: SourcePosition(params.position, in: source), reportingDiagnosticsTo: logger)
    else {
  ...
  private func buildCompletion(
    ...
    if n.name.value.identifier == dummyNode {
      ...
      if let q: ExpressionIdentity = n.qualification {
        return .success(TwoTypeOption.optionB(CompletionList(from: q, in: program, log: logger)))
      } else {
